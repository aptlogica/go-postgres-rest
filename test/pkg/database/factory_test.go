...
func TestDatabaseConnectorFactoryCreateConnection(t *testing.T) {
	...
	factory.RegisterConnector("postgres", databasepkg.NewPostgresConnectionFactory(dsnBuilder, conn))
...
func TestPostgresConnectionFactoryCreateConnection(t *testing.T) {
	...
	t.Run("builder error", func(t *testing.T) {
		...
		factory := databasepkg.NewPostgresConnectionFactory(builder, &mockConnector{})
	...
	t.Run("connect error", func(t *testing.T) {
		...
		factory := databasepkg.NewPostgresConnectionFactory(builder, conn)
...
func TestNewDefaultDatabaseConnectorFactoryRegistersPostgres(t *testing.T) {
	factory := databasepkg.NewDefaultDatabaseConnectorFactory()
	cfg := &configpkg.DatabaseConfig{}

	// use a short-lived connector to avoid hitting real DB by overriding map
	mockConn := &mockConnector{db: mockDB{}}
	factory.RegisterConnector("postgres", databasepkg.NewPostgresConnectionFactory(&mockDSNBuilder{dsn: "dsn"}, mockConn))

	if _, err := factory.CreateConnection("postgres", cfg); err != nil {
		t.Fatalf("expected postgres connector to be registered: %v", err)
	}
}

// Ensure mock types conform during compile time
var _ databasepkg.ConnectionFactory = (*databasepkg.PostgresConnectionFactory)(nil)
