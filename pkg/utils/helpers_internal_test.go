...
func TestContainsHelpers(t *testing.T) {
	...
	if !ContainsInt([]int{1, 2}, 2) || ContainsInt([]int{1}, 2) {
		t.Fatalf("ContainsInt failed")
	}
	if !ContainsInt64([]int64{1, 2}, 2) || ContainsInt64([]int64{1}, 2) {
		t.Fatalf("ContainsInt64 failed")
	}
	if !Contains([]byte{1, 2}, 2) || Contains([]byte{1}, 2) {
		t.Fatalf("Contains generic failed")
	}
	if !ContainsLegacy([]int{1, 2}, 2) || ContainsLegacy([]int{1}, 2) {
		t.Fatalf("ContainsLegacy failed")
	}
}

func TestRemoveDuplicates(t *testing.T) {
	if got := RemoveDuplicatesString([]string{"a", "b", "a"}); !reflect.DeepEqual(got, []string{"a", "b"}) {
		t.Fatalf("RemoveDuplicatesString mismatch: %v", got)
	}
	if got := RemoveDuplicatesInt([]int{1, 2, 1}); !reflect.DeepEqual(got, []int{1, 2}) {
		t.Fatalf("RemoveDuplicatesInt mismatch: %v", got)
	}
	if got := RemoveDuplicates([]byte{1, 1, 2}); !reflect.DeepEqual(got, []byte{1, 2}) {
		t.Fatalf("RemoveDuplicates generic mismatch: %v", got)
	}
	if got, ok := RemoveDuplicatesLegacy([]interface{}{1, 1, 2}).([]interface{}); !ok || len(got) != 2 {
		t.Fatalf("RemoveDuplicatesLegacy mismatch: %#v", got)
	}
}

func TestStringHelpers(t *testing.T) {
	if TruncateString("hello", 2) != "he" {
		t.Fatalf("TruncateString short failed")
	}
	if TruncateString("hello", 5) != "hello" {
		t.Fatalf("TruncateString equal failed")
	}
	if TruncateString("hello", 4) != "h..." {
	...
	if got := FormatFileSize(5*1024*1024 + 100); got != "5.0 MB" {
		t.Fatalf("FormatFileSize large mismatch: %s", got)
	}

	if got := SliceToStringStrings([]string{"a", "b"}); got != "a, b" {
		t.Fatalf("SliceToStringStrings mismatch: %s", got)
	}
	if got := SliceToStringInts([]int{1, 2}); got != "1, 2" {
		t.Fatalf("SliceToStringInts mismatch: %s", got)
	}
	if got := SliceToString([]int{1, 2}); got != "1, 2" {
		t.Fatalf("SliceToString mismatch: %s", got)
	}
	if got := StringToSlice("a, b"); !reflect.DeepEqual(got, []string{"a", "b"}) {
...
func TestReverseHelpers(t *testing.T) {
	...
	var notSlice = 123
	ReverseLegacy(notSlice) // should not panic
}
